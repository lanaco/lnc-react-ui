...
// Test if nested layout can go deeper than 2 levels
const Layout = (props) => {
  const { children, isChild, __TYPE__, ...rest } = props;
  const theme = useTheme();

  const hasSidebar = React.Children.toArray(children).some(component => component.props.__TYPE__ == "Sidebar");
  const getSidebarPlacement = (children) => {
    ...
    React.Children.forEach(children, (component, index) => {
      if (component.props.__TYPE__ == "Sidebar") {
        ...
        if (layoutIndex) throw 'Found multiple ocurrences of Layout component on the same level in the component tree.'
        layoutIndex = index + 1;
      }
  ...
  const clonedChildren = React.Children.map(children, child => {
    if (React.isValidElement(child)) {
      if (child.props.__TYPE__ == "Sidebar")
        return React.cloneElement(child, { placement: getSidebarPlacement(children) });
  ...
  return (
    <StyledLayout isChild={isChild} hasSidebar={hasSidebar} theme={theme} {...rest}>
      {clonedChildren}
    </StyledLayout>
  )
}

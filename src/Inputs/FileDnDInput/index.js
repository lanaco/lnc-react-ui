...
var paddingBySize = { small: "8px 13px", medium: "9.5px 14.5px", large: "11.75px 16.75px" };
...
const Container = styled.div`
  ...
  border: 1px dashed 
  ${(props) => props.theme.test_palette.light[500]};
  border-radius: 5px;
...
const FileDnDInput = React.forwardRef((props, ref) => {
    const {
        id,
        className,
        style,
    ...
    const { getRootProps, getInputProps } = useDropzone({
        onDrop: handleOnDrop, accept: accept, onDropAccepted: handleOnDropAccepted
    })

    return (
        <Container {...themeProps} className={className} style={style} {...getRootProps()}>
            <Input
                {...getInputProps()}
                {...themeProps}
                multiple={multiple}
